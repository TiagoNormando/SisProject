...
procedure TfCadCliente.Bloqueio(Tipo: Boolean);
begin
...
procedure TfCadCliente.btnExportarClick(Sender: TObject);
begin
  try
          Bloqueio(False);
          //
...
procedure TfCadCliente.btnIncluirClick(Sender: TObject);
begin
     // Inserir Registro
     if (srcRegistro.State = dsBrowse) and tshListagem.TabVisible then
          tshListagem.SetFocus;
...
procedure TfCadCliente.btnLocalizarClick(Sender: TObject);
var
     Contador: Integer;
begin
...
procedure TfCadCliente.cbxOpcaoChange(Sender: TObject);
var
     Contador: Integer;
begin
...
procedure TfCadCliente.DBEditCodigoKeyPress(Sender: TObject; var Key: Char);
begin
     // [ENTER] => [TAB]
     if (Key = #13) then
     begin
...
procedure TfCadCliente.DBGridRegistroDblClick(Sender: TObject);
begin
    if (srcRegistro.State = dsBrowse) and (TClientDataSet(srcRegistro.DataSet).RecordCount > 0) and (btnEditar.Enabled) then
          btnEditar.Click;
end;

procedure TfCadCliente.DBGridRegistroDrawColumnCell(Sender: TObject;
...
procedure TfCadCliente.DBGridRegistroTitleClick(Column: TColumn);
var
     Contador: Integer;
     Agregado: Boolean;
begin
...
procedure TfCadCliente.FormResize(Sender: TObject);
 var
     Contador: Integer;
     Tamanho: Integer;
begin
...
procedure TfCadCliente.srcRegistroDataChange(Sender: TObject; Field: TField);
begin
  // Verificar Status
